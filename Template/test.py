...
img_length_width =224  #224 for VGG. 256 for others
...
def parse_args():
    ...
    args.add_argument("--test_data_dir", default = "D:\QQPCMgr(1)\Desktop\COMP309_2019_Project\Train_data",
                      help = "path to test_data_dir")
    args = vars(args.parse_args())
    return args


def load_images(test_data_dir, image_size = (img_length_width, img_length_width)):
    """
...
def evaluate(X_test):
    ...
    ******Please do not change this function******
    ...
    # Load Model
    model = load_model('D:\QQPCMgr(1)\Desktop\COMP309_2019_Project\Template\model\VGG16model(goodone).h5')
    return model.evaluate_generator(X_test,steps=len(X_test), verbose = 1)
...
if __name__ == '__main__':
    ...
    image_size = (img_length_width, img_length_width)

    # Load images
    images_generator = load_images(test_data_dir, image_size)

    # Evaluation, please make sure that your training model uses "accuracy" as metrics, i.e., metrics=['accuracy']
    loss, accuracy = evaluate(images_generator)
