...
def construct_model(model_type ='model type'):
    ...
    ***
    ...
    # 2. CNN model
        ...
        model.add(Dense(3,activation='softmax'))
        optimizer= Adam(lr=0.001)#RMSprop(lr=0.0001,rho=0.9) #Adam(lr=0.001)
        model.compile(optimizer=optimizer,loss='categorical_crossentropy',metrics=['accuracy']) # try rmsprop
        learning_rate_reduction=callbacks.ReduceLROnPlateau(monitor='val_acc',patience=3,verbose=1,factor=0.5,minlr=0.00001)
        model.summary()
    return model
...
if __name__ == '__main__':
    ...
    plt.savefig('Visualization/Accuracy_{}.png'.format(int(time.time())))
    #plot loss curve
    plt.clf()
    ...
    plt.xlabel('Epoch')
    plt.legend(['train','test'],loc='upper left')
    plt.savefig('Visualization/loss_{}.png'.format(int(time.time())))